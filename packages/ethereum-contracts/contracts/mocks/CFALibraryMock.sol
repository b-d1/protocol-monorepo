...
contract CFALibraryMock {
    ...
    function createFlowTest(ISuperfluidToken token, address receiver, int96 flowRate) public {
        cfaV1.createFlow(receiver, token, flowRate);
    }

    function updateFlowTest(ISuperfluidToken token, address receiver, int96 flowRate) public {
    ...
    function createFlowByOperatorTest(address sender, address receiver, ISuperfluidToken token, int96 flowRate)
        public
    {
        cfaV1.createFlowByOperator(sender, receiver, token, flowRate);
    }

    function updateFlowByOperatorTest(address sender, address receiver, ISuperfluidToken token, int96 flowRate)
        public
    {
        cfaV1.updateFlowByOperator(sender, receiver, token, flowRate);
    }

    function deleteFlowByOperator(address sender, address receiver, ISuperfluidToken token) public {
    ...
    function authorizeFlowOperatorWithFullControlTest(address flowOperator, ISuperfluidToken token) public {
        cfaV1.authorizeFlowOperatorWithFullControl(flowOperator, token);
    }

    function revokeFlowOperatorWithFullControlTest(address flowOperator, ISuperfluidToken token) public {
...
contract CFALibrarySuperAppMock is SuperAppBase {
    ...
    constructor(ISuperfluid host, address defaultSender, address defaultReceiver, address defaultFlowOperator) {
        cfaV1 = CFAv1Library.InitData(
            host,
            IConstantFlowAgreementV1(
                address(host.getAgreementClass(keccak256("org.superfluid-finance.agreements.ConstantFlowAgreement.v1")))
    ...
    function afterAgreementCreated(
        ...
        if (functionIndex == uint8(FunctionIndex.CREATE_FLOW)) {
            ...
            return cfaV1.updateFlowOperatorPermissionsWithCtx(ctx, flowOperator, token, PERMISSIONS, FLOW_RATE);
