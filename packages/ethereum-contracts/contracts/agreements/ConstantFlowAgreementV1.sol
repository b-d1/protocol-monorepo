...
contract ConstantFlowAgreementV1 is AgreementBase, IConstantFlowAgreementV1 {
        ...
        returns (bool)
    {
        ...
        return _isPatricianPeriod(availableBalance, signedTotalCFADeposit, liquidationPeriod, patricianPeriod);
    }
    ...
    function _handleOnCreateHook(_StackVars_createOrUpdateFlow memory flowVars) internal {
        uint256 gasLeftBefore = gasleft();

        address constantOutflowNFTAddress = _canCallNFTHook(flowVars.token);

        if (constantOutflowNFTAddress != address(0)) {
            try IConstantOutflowNFT(constantOutflowNFTAddress).onCreate(
    ...
    function _handleOnUpdateHook(_StackVars_createOrUpdateFlow memory flowVars) internal {
        uint256 gasLeftBefore = gasleft();

        address constantOutflowNFTAddress = _canCallNFTHook(flowVars.token);

        if (constantOutflowNFTAddress != address(0)) {
            try IConstantOutflowNFT(constantOutflowNFTAddress).onUpdate(
                flowVars.token, flowVars.sender, flowVars.receiver
            ) {
                // solhint-disable-next-line no-empty-blocks
            } catch {
    ...
    function _handleOnDeleteHook(_StackVars_createOrUpdateFlow memory flowVars) internal {
        uint256 gasLeftBefore = gasleft();

        address constantOutflowNFTAddress = _canCallNFTHook(flowVars.token);

        if (constantOutflowNFTAddress != address(0)) {
            try IConstantOutflowNFT(constantOutflowNFTAddress).onDelete(
                flowVars.token, flowVars.sender, flowVars.receiver
            ) {
                // solhint-disable-next-line no-empty-blocks
            } catch {
    ...
    function increaseFlowRateAllowance(
        ...
        _updateFlowRateAllowance(token, flowOperatorId, oldPermissions, newFlowRateAllowance);

        emit FlowOperatorUpdated(token, currentContext.msgSender, flowOperator, oldPermissions, newFlowRateAllowance);
    }
    ...
    function _changeFlowToApp(
        ...
        if (appToCallback != address(0)) {
            ...
            uint256 minimumDeposit =
                gov.getConfigAsUint256(ISuperfluid(msg.sender), token, SUPERTOKEN_MINIMUM_DEPOSIT_KEY);
    ...
    function _changeFlow(
        ...
        uint256 newDeposit;
        {
            ...
            // STEP 1: calculate deposit required for the flow
            {
                (uint256 liquidationPeriod,) = _decode3PsData(token);
                ISuperfluidGovernance gov = ISuperfluidGovernance(ISuperfluid(msg.sender).getGovernance());
                minimumDeposit = gov.getConfigAsUint256(ISuperfluid(msg.sender), token, SUPERTOKEN_MINIMUM_DEPOSIT_KEY);
    ...
    function _makeLiquidationPayouts(
        ...
        // Note: curly brackets are to handle stack too deep overflow issue
        {
            ...
            isCurrentlyPatricianPeriod =
                _isPatricianPeriod(availableBalance, signedTotalCFADeposit, liquidationPeriod, patricianPeriod);
        }
